...
*   Python에서는 `aiokafka`의 특성상 모든 커밋이 기본적으로 비동기입니다. `await consumer.commit()`도 실제로는 비동기 방식으로 동작하지만 결과를 기다립니다.
...
*   **재조정 (Rebalancing) 처리**:
    ...
    *   대부분의 카프카 클라이언트 라이브러리는 `ConsumerRebalanceListener` (Java/Scala) 또는 이와 유사한 콜백 메커니즘(`aiokafka`의 `subscribe` 메서드 `listener` 인자)을 제공합니다. 이를 사용하여 파티션 할당이 취소되기 전(`on_partitions_revoked`)에 안전하게 현재까지 처리된 오프셋을 커밋하고, 새로운 파티션이 할당된 후(`on_partitions_assigned`) 필요한 초기화 작업을 수행해야 합니다.
...
*   **오류 처리 및 재시도 로직**:
    ...
    *   일시적인 오류라면 재시도(retry) 로직을 구현할 수 있습니다. 재시도 횟수 제한, 재시도 간격(exponential backoff 등) 등을 설정해야 합니다.
    *   반복적으로 실패하는 메시지(소위 '독이 든 메시지', Poison Pill)는 별도의 Dead Letter Queue(DLQ)로 보내거나, 오류를 로깅하고 해당 메시지를 건너뛰는 등의 처리가 필요합니다. 그렇지 않으면 해당 메시지에서 컨슈머가 계속 멈춰있게 됩니다.
